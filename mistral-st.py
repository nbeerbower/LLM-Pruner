...
import torch
import numpy as np
from transformers import LlamaTokenizer, GenerationConfig, LlamaConfig, AutoTokenizer, LlamaForCausalLM, MistralForCausalLM
...
def main(args):
    ...
    if args.block_wise:
        kwargs = {
            ...
            "customized_pruners": {
                MistralRMSNorm: llama_pruner.hf_rmsnorm_pruner,  # Reuse Llama pruner as structure is similar
            },
            "root_module_types": None,
            # Adjust layer ranges based on Mistral's 40 layers
            "root_instances": [model.model.layers[i].self_attn.k_proj for i in range(args.block_attention_layer_start, min(args.block_attention_layer_end, 40))] +
                            [model.model.layers[i].mlp.gate_proj for i in range(args.block_mlp_layer_start, min(args.block_mlp_layer_end, 40))]
        }
    ...
    if args.save_model:
        ...
        output_dir = os.path.dirname(logger.best_checkpoint_path)
        os.makedirs(output_dir, exist_ok=True)
        
        print("Save model in safetensors format to {}".format(output_dir))
